 ...
 * depth), parameterized by the world seed ({@link WorldSeeds} phases 13–16),
 * so cave biome patches — like heights — agree across every seam by
...
public final class CaveBiomes {
    ...
    public CaveBiomes(WorldSeeds seeds) {
        this.seeds = seeds;
    }
     ...
     */
    public CaveBiome at(Vec3 p, int y) {
        if (y > CAVE_BIOME_MAX_Y) {
            return CaveBiome.NONE;
        }
        double n = Math.sin(7.3 * p.x() + seeds.phase(13)) * Math.cos(6.1 * p.z() + seeds.phase(14))
                + Math.sin(5.7 * p.y() + seeds.phase(15))
                + Math.sin(0.21 * y + 3.0 * p.x() - 2.0 * p.z() + seeds.phase(16));
        if (y < -25 && n < -1.7) {
