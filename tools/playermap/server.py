...
<style>
  ...
  .pmk{position:absolute;will-change:left,top}
...
<a href="/guide" style="position:fixed;top:58px;right:16px;z-index:11;font:600 13px system-ui;
 color:#cdd6e4;background:rgba(12,16,24,.6);padding:8px 12px;border-radius:10px;
 border:1px solid rgba(120,150,190,.18);text-decoration:none">what is this place?</a>
<script>
(function(){
  ...
  function loop(){
    ...
    for(var k=0;k<players.length;k++){
      ...
      // anchor the DOT'S CENTER on the projected point (the old -100% pinned
      // the element bottom there, floating the dot a label-height "north").
      el.style.transform='translate(-50%,-'+(px/2+2)+'px)';
    ...
    def do_GET(self):
        try:
            ...
            if self.path.startswith("/guide"):
                self._send(guide.PAGE.encode("utf-8"), "text/html; charset=utf-8")
