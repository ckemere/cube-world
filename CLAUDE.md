...
- **Fresh-world bring-up (3 steps — automate, don't hand-drive):** (1) `./run-server.sh` — first boot logs "Village anchor: no cubeworld structure sets found" (it writes the cities datapack during this boot). (2) Restart (RCON `stop`, `./run-server.sh`) — the second boot logs "Village anchor: 30 cities anchored". (3) `python3 tools/seed_cities.py` — force-loads all 30 city chunks so every teleport **station** builds and both rings fill to 30/30, then saves + clears force-loads. Without step 3 the teleport network shows "The network is still forming" because stations build lazily (only visited cities register). Also restart the map webserver if used: `cd tools/playermap && python3 server.py` (:8080).
